...
export function NewHabitForm() {
  ...
  function handleToggleWeekDay(weekDayIndex: number, shouldAddToList: boolean) {
    setWeekDays(
      prevState => shouldAddToList
    ...
    <form onSubmit={createNewHabit} className="w-full flex flex-col mt-6">
      ...
      <div className="flex flex-col gap-2 mt-3">
        {availableWeekDays.map((weekDay, index) => (
          <Checkbox.Root
            key={weekDay}
            className="flex items-center gap-3 p-0 group focus:outline-none"
            checked={weekDays.includes(index)}
            onCheckedChange={(checked: boolean) => handleToggleWeekDay(index, checked)}
          >
